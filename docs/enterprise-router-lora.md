...
当前正式训练数据由仓库内的确定性生成器构造，业务主体是虚构的 Northstar Labs，
不读取 Redwood 文档或官方问题。生成器覆盖 10 类问题和 9 种数据来源；两者具有不同：
...
仓库执行 ID、规范化文本哈希和两两 token Jaccard 检查。V2 的 2,000 条训练公司问题
与 500 条官方问题无精确重合，最大 token Jaccard 为 0.233333（失败阈值 0.8）。
模板合成数据的语言多样性是明确局限，后续可用独立本地模型扩写后再做近邻去重。
