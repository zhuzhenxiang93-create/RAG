...
当前版本已完成无密钥可运行的文档接入、意图路由、混合检索、可信问答、可选领域插件、
...
主系统默认启用 CPU 可运行的 Lite 意图基线；Full 模式可切换为在 MASSIVE
中文或中英数据上训练的 Qwen2.5 LoRA 序列分类器。分类结果不是孤立展示，而是进入
RAG 检索计划：

`用户问题 → 领域/意图 → 知识库建议 → 检索权重 → 证据 → 回答`

- `GET /api/plugins/intent/status`
- `POST /api/plugins/intent/classify`
- `POST /api/search` 和 `POST /api/chat` 的 `plan` 返回意图、领域、置信度、
  知识库建议、路由后检索权重与低置信度回退状态。

Lite 基线只用于离线演示和系统降级，不是训练模型，也没有对外宣称准确率。完整的数据
下载、LoRA 训练、测试集评测和配置步骤见 `docs/intent-lora.md`。
...
法律能力只作为历史领域扩展示例，已从项目主叙事降级；面试主线是通用企业知识服务和
意图驱动的 RAG 路由。
